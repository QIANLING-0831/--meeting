...
let audioStatuses = {interviewer:"", candidate:""};
...
$("startButton").onclick=async()=>{try{await api("/api/interview/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({microphone_enabled:$("microphone").checked,microphone_device_id:$("microphoneDevice").value,loopback_device_name:$("loopbackDevice").value})});setRunning(true)}catch(e){toast(e.message,true)}};
$("stopButton").onclick=async()=>{try{await api("/api/interview/stop",{method:"POST"});setRunning(false)}catch(e){toast(e.message,true)}};
function setRunning(on){$("liveBadge").textContent=on?"识别中":"未开始";$("liveBadge").className=`live-dot ${on?"on":""}`;$("startButton").disabled=on;$("stopButton").disabled=!on;$("loopbackDevice").disabled=on;if(!on){audioStatuses={interviewer:"",candidate:""};["interviewer","candidate"].forEach(prefix=>{$(prefix+"Level").value=0;$(prefix+"LevelText").textContent="未检测"})}}
...
function handleEvent(event){
  ...
  if(event.type==="audio_level"){const prefix=event.speaker==="interviewer"?"interviewer":"candidate";const level=Math.max(0,Math.min(1,Number(event.level)||0));$(prefix+"Level").value=level;if(!audioStatuses[prefix])$(prefix+"LevelText").textContent=level<.18?"偏小":level>.82?"过大":"正常"}
  if(event.type==="audio_status"){const prefix=event.speaker==="interviewer"?"interviewer":"candidate";audioStatuses[prefix]=event.status==="connected"?"":event.status;if(event.status==="silent")$(prefix+"LevelText").textContent="无音频·检查设备";if(event.status==="reconnecting")$(prefix+"LevelText").textContent="重连中";if(event.status==="connected"&&event.message)$(prefix+"LevelText").textContent="已连接"}

...
$("microphone").onchange=()=>$("microphoneDevice").classList.toggle("hidden",!$("microphone").checked);
...
function setRunning(on){$("liveBadge").textContent=on?"识别中":"未开始";$("liveBadge").className=`live-dot ${on?"on":""}`;$("startButton").disabled=on;$("stopButton").disabled=!on}
...
function connect(){const ws=new WebSocket(`${location.protocol==="https:"?"wss":"ws"}://${location.host}/ws`);ws.onmessage=e=>handleEvent(JSON.parse(e.data));ws.onclose=()=>setTimeout(connect,1500)}
function handleEvent(event){
  ...
  if(event.type==="knowledge_hits"){currentQuestion=event.question;$("questionBox").textContent=currentQuestion;const wrap=$("knowledgeHits");wrap.replaceChildren();event.hits.slice(0,1).forEach(h=>{const card=document.createElement("div");card.className="hit";const badge=document.createElement("b");badge.textContent=`${h.pack} · 题库速答`;const snippet=document.createElement("span");snippet.textContent=(h.answer_preview||h.content).slice(0,620);card.append(badge,snippet);wrap.append(card)})}
